...
function GoalsView({ goal, onSetGoal, onClearGoal, entries, userId = DEFAULT_USER_ID }) {
    ...
    useEffect(() => {
        ...
        const fetchProgress = async () => {
            try {
                const progRes = await fetch(`/goals/${goal.goalId}/progress`);
                if (progRes.ok) {
                    const prog = await progRes.json();
                    setGoalProgressData(prog);
                }
            } catch (err) {
                console.warn("Backend API offline for goal progress. Using local calculation.");
            }
        };
        fetchProgress();
    }, [goal]);
    ...
    const progress = useMemo(() => {
        ...
        return {
            requiredPct, actualPct, onTrack, achieved, daysRemaining, targetWeekly, baselineWeekly: goal.baselineWeekly
        };
...
function BadgesView({ earnedIds }) {
    const T = useTheme();
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <Card>
                <SectionLabel>Milestone Badges</SectionLabel>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
                    {BADGES.map((b) => {
                        const isUnlocked = earnedIds.includes(b.id);
                        const Icon = b.icon;
                        return (
                            <div key={b.id} style={{ border: `1px solid ${T.line}`, borderRadius: 10, padding: 16, background: isUnlocked ? T.paperRaised : T.card, opacity: isUnlocked ? 1 : 0.65 }}>
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                                    <div style={{ width: 36, height: 36, borderRadius: "50%", background: isUnlocked ? b.color(T) : T.grey, display: "flex", alignItems: "center", justifyContent: "center", color: T.paper }}>
                                        <Icon size={18} />
                                    </div>
                                    {isUnlocked ? <CheckCircle2 size={16} color={T.moss} /> : <Lock size={16} color={T.grey} />}
                                </div>
                                <div style={{ fontFamily: BODY, fontSize: 14, fontWeight: 700, color: T.ink }}>{b.name}</div>
                                <div style={{ fontFamily: BODY, fontSize: 12, color: T.subtext, marginTop: 4 }}>{b.desc}</div>
                                <div style={{ fontFamily: MONO, fontSize: 10, color: isUnlocked ? T.moss : T.subtext, marginTop: 10 }}>{b.requirement}</div>
                            </div>
                        );
                    })}
                </div>
...
function LeaderboardView({ youWeekly }) {
    const T = useTheme();
    const sorted = useMemo(() => {
        const list = LEADERBOARD.map((item) => item.name === "You" ? { ...item, weekly: youWeekly } : item);
        return list.sort((a, b) => (a.weekly || 0) - (b.weekly || 0));
    }, [youWeekly]);

    return (
        <Card>
            <SectionLabel>Community Leaderboard · Weekly Output</SectionLabel>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
                {sorted.map((item, index) => {
                    const isYou = item.name === "You";
                    return (
                        <div key={item.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderRadius: 8, background: isYou ? T.paperRaised : T.paper, border: isYou ? `1px solid ${T.moss}` : `1px solid ${T.line}` }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                <span style={{ fontFamily: MONO, fontSize: 14, fontWeight: 700, width: 24, color: T.subtext }}>#{index + 1}</span>
                                <span style={{ fontFamily: BODY, fontSize: 14, fontWeight: isYou ? 700 : 500, color: T.ink }}>{item.name}</span>
                            </div>
                            <span style={{ fontFamily: MONO, fontSize: 14, color: T.ink }}>{fmt1(item.weekly)} kg</span>
                        </div>
                    );
                })}
            </div>
...
function TeamReportView() {
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <Card>
                <SectionLabel>Departmental Averages</SectionLabel>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
                    {DEPARTMENTS.map((d) => (
                        <div key={d.dept} style={{ padding: 14, background: T.paper, borderRadius: 8, border: `1px solid ${T.line}` }}>
                            <div style={{ fontFamily: BODY, fontSize: 13, color: T.subtext }}>{d.dept}</div>
                            <div style={{ fontFamily: DISPLAY, fontSize: 22, color: T.ink, marginTop: 4 }}>{d.avg} <span style={{ fontSize: 12, fontFamily: BODY }}>kg/wk</span></div>
                        </div>
                    ))}
                </div>
            </Card>
            ...
            <Card>
                <SectionLabel>Team Members Overview</SectionLabel>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {TEAM_MEMBERS.map((m) => (
                        <div key={m.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: T.paper, borderRadius: 8, border: `1px solid ${T.line}` }}>
                            <div>
                                <div style={{ fontFamily: BODY, fontSize: 13, fontWeight: 600, color: T.ink }}>{m.name}</div>
                                <div style={{ fontFamily: BODY, fontSize: 11, color: T.subtext }}>{m.dept}</div>
                            </div>
                            <div style={{ textAlign: "right" }}>
                                <div style={{ fontFamily: MONO, fontSize: 13, color: T.ink }}>{m.weekly} kg</div>
                                <Trend value={m.trend} />
                            </div>
                        </div>
                    ))}
...
/* ---------------------------------------------------------------- */
/* Main Dashboard Component Engine                                  */
/* ---------------------------------------------------------------- */
export default function Dashboard() {
    ...
    const allEntries = useMemo(() => [...activities, ...history], [activities, history]);
    const todayTotal = useMemo(() => activities.reduce((s, a) => s + a.co2e, 0), [activities]);

    const weeklyBuckets = useMemo(() => aggregate(allEntries, "week", 1), [allEntries]);
    const youWeekly = weeklyBuckets[0]?.total || todayTotal;

    const rank = useMemo(() => {
        const sorted = [...LEADERBOARD.map((i) => i.name === "You" ? { ...i, weekly: youWeekly } : i)].sort((a, b) => (a.weekly || 0) - (b.weekly || 0));
        return sorted.findIndex((i) => i.name === "You") + 1;
    }, [youWeekly]);

    const quests = [
        { key: "q1", label: "Log today's primary commute activity", done: activities.some((a) => a.category === "Transport"), tab: "log" },
        { key: "q2", label: "Log at least one diet/food entry", done: activities.some((a) => a.category === "Food"), tab: "log" },
        { key: "q3", label: "Review personalized reduction recommendations", done: activeTab === "insights", tab: "insights" },
    ];

    const earnedIds = useMemo(() => {
        const list = ["streak_7"];
        if (quests.some((q) => q.done)) list.push("first_goal");
        if (todayTotal < 15) list.push("reduction_10");
        return list;
    }, [quests, todayTotal]);
    ...
    return (
        <ThemeContext.Provider value={theme}>
            <div style={{ minHeight: "100vh", background: theme.paper, color: theme.ink, fontFamily: BODY, transition: "background 0.2s ease" }}>
                <TopNav activeTab={activeTab} onTab={setActiveTab} dark={dark} onToggleDark={() => setDark(!dark)} />

                <main style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px 80px 20px" }}>
                    {activeTab === "dashboard" && (
                        <DashboardHome activities={activities} todayTotal={todayTotal} rank={rank} quests={quests} onNavigate={setActiveTab} earnedIds={earnedIds} />
                    )}
                    ...
                    {activeTab === "goals" && (
                        <GoalsView goal={goal} onSetGoal={setGoal} onClearGoal={() => setGoal(null)} entries={allEntries} userId={DEFAULT_USER_ID} />
                    )}
                    ...
                    {activeTab === "badges" && (
                        <BadgesView earnedIds={earnedIds} />
                    )}

                    {activeTab === "leaderboard" && (
                        <LeaderboardView youWeekly={youWeekly} />
                    )}
                ...
                <SproutChat allEntries={allEntries} todayTotal={todayTotal} goal={goal} youWeekly={youWeekly} />
            </div>
