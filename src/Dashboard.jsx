...
function SectionLabel({ children }) {
    return (
        <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: C.subtext, marginBottom: 10 }}>
            {children}
        </div>
    );
}
...
function Card({ children, style }) {
    return (
        <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 10, padding: 20, ...style }}>
            {children}
        </div>
    );
}
        ...
        <Card>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
                <div style={{ flex: "1 1 200px" }}>
                    <label style={{ fontFamily: BODY, fontSize: 12, color: C.subtext, display: "block", marginBottom: 6 }}>Activity type</label>
                    <select
                        value={activityType}
                        onChange={(e) => setActivityType(e.target.value)}
                        style={{ width: "100%", padding: "9px 10px", fontFamily: BODY, fontSize: 14, color: C.ink, background: C.paper, border: `1px solid ${C.line}`, borderRadius: 6 }}
                    >
                ...
                <div style={{ flex: "0 0 140px" }}>
                    <label style={{ fontFamily: BODY, fontSize: 12, color: C.subtext, display: "block", marginBottom: 6 }}>Quantity ({factorInfo.unit})</label>
                    <input
                        type="number"
                        min="0"
                        step="0.1"
                        ...
                        placeholder="0"
                        style={{ width: "100%", padding: "9px 10px", fontFamily: MONO, fontSize: 14, color: C.ink, background: C.paper, border: `1px solid ${C.line}`, borderRadius: 6, boxSizing: "border-box" }}
                    />
                </div>
            </div>

            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: C.paper, borderRadius: 8, marginBottom: 18 }}>
            ...
            <button
                onClick={submit}
                disabled={qtyNum <= 0}
                style={{ width: "100%", padding: "11px 0", fontFamily: BODY, fontSize: 14, fontWeight: 600, color: C.paper, background: qtyNum > 0 ? C.moss : C.grey, border: "none", borderRadius: 6, cursor: qtyNum > 0 ? "pointer" : "not-allowed", marginBottom: 22 }}
            >
            ...
            <div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                        <button onClick={() => setCarouselStart(Math.max(0, carouselStart - 1))} disabled={carouselStart === 0} style={{ border: `1px solid ${C.line}`, background: C.paper, borderRadius: 5, padding: 3, cursor: "pointer" }}>
                            <ChevronLeft size={14} color={C.subtext} />
                        </button>
                        <button onClick={() => setCarouselStart(Math.min(QUICK_LOG.length - 3, carouselStart + 1))} disabled={carouselStart >= QUICK_LOG.length - 3} style={{ border: `1px solid ${C.line}`, background: C.paper, borderRadius: 5, padding: 3, cursor: "pointer" }}>
                    ...
                    {visible.map((preset) => {
                        ...
                        return (
                            <button
                                ...
                                style={{ flex: 1, textAlign: "left", padding: "10px 12px", background: C.paper, border: `1px dashed ${C.line}`, borderRadius: 8, cursor: "pointer" }}
                            >
            ...
            <div style={{ position: "relative", height: 240 }}>
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        ...
                        <Tooltip contentStyle={{ fontFamily: BODY, fontSize: 12, border: `1px solid ${C.line}`, borderRadius: 6 }} formatter={(v) => [`${fmt1(v)} kg CO₂e`, ""]} />
                        <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontFamily: BODY, fontSize: 12, color: C.subtext }} />
                    </PieChart>
                </ResponsiveContainer>
            ...
            <div style={{ height: 220 }}>
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <Tooltip contentStyle={{ fontFamily: BODY, fontSize: 12, border: `1px solid ${C.line}`, borderRadius: 6 }} formatter={(v) => (v == null ? ["—", ""] : [`${fmt1(v)} kg`, ""])} />
    ...
    return (
        <Card>
            ...
            <div style={{ position: "relative", height: 14, background: C.paper, borderRadius: 999, border: `1px solid ${C.line}` }}>
                <div style={{ position: "absolute", top: -1, left: 0, height: 14, width: `${pct}%`, background: `linear-gradient(90deg, ${C.lichen}, ${over ? C.danger : C.moss})`, borderRadius: 999, transition: "width 0.4s ease" }} />
        ...
        <Card>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                    {["Friends", "Global"].map((s) => (
                        <button
                            key={s}
                            onClick={() => setScope(s)}
                            style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, padding: "5px 10px", borderRadius: 999, border: `1px solid ${scope === s ? C.moss : C.line}`, background: scope === s ? C.moss : "transparent", color: scope === s ? C.paper : C.subtext, cursor: "pointer" }}
                        >
                            {s}
                        </button>
                    ))}
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
                {ranked.map((p, i) => {
                    ...
                    return (
                        <div key={p.name} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 8px", borderRadius: 6, background: isYou ? C.paper : "transparent", border: isYou ? `1px solid ${C.moss}` : "1px solid transparent" }}>
                            <span style={{ fontFamily: MONO, fontSize: 12, color: i < 3 ? C.moss : C.subtext, fontWeight: i < 3 ? 700 : 400, width: 20 }}>#{i + 1}</span>
                            <div style={{ width: 26, height: 26, borderRadius: "50%", background: isYou ? C.moss : C.lichen, color: C.paper, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: BODY, fontSize: 11, fontWeight: 700 }}>
                                {p.name.split(" ").map((w) => w[0]).join("")}
                            </div>
                            <span style={{ fontFamily: BODY, fontSize: 13, fontWeight: isYou ? 700 : 500, color: C.ink, flex: 1 }}>{p.name}</span>
                ...
                <div style={{ height: 220 }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={DEPARTMENTS} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <YAxis tick={{ fontFamily: BODY, fontSize: 11, fill: C.subtext }} axisLine={false} tickLine={false} />
                            <Tooltip contentStyle={{ fontFamily: BODY, fontSize: 12, border: `1px solid ${C.line}`, borderRadius: 6 }} formatter={(v) => [`${fmt1(v)} kg`, "avg"]} />
...
export default function Dashboard({ setAuth }) {
    ...
    // 📁 Inside Dashboard.jsx -> Find handleLogActivity and update it to this:
    const handleLogActivity = async (newAct) => {
        try {
            const todayStr = new Date().toISOString().split('T')[0];
            const userId = localStorage.getItem('userId') || 1;

            // Provide all common Java field variations for safety
            const savedActivity = await activityAPI.logActivity({
            ...
            if (savedActivity) {
                // Refresh dashboard view immediately
                await fetchDashboardData();
            }
        } catch (error) {
            alert("Error saving activity to database.");
            console.error(error);
        }
    };
    ...
    if (isLoading) {
        return (
            <div style={{ ...styles.appContainer, display: "flex", alignItems: "center", justifyContent: "center", height: "100vh" }}>
                <div style={{ fontFamily: DISPLAY, fontSize: "1.5rem", color: C.moss }}>Loading carbon dashboard metrics...</div>
            </div>
        );
    }
        ...
        <div style={styles.appContainer}>
            <header style={styles.header}>
                ...
                <div style={styles.navActions}>
                    <button onClick={() => setView("Personal")} style={{ ...styles.toggleBtn, background: view === "Personal" ? C.moss : "transparent", color: view === "Personal" ? C.paper : C.subtext, borderColor: view === "Personal" ? C.moss : C.line }}>👤 Personal</button>
                    <button onClick={() => setView("Team")} style={{ ...styles.toggleBtn, background: view === "Team" ? C.moss : "transparent", color: view === "Team" ? C.paper : C.subtext, borderColor: view === "Team" ? C.line : C.line }}>👥 Team report</button>
                    <div style={styles.profileContainer}>
                        <button onClick={() => setIsMenuOpen(!isMenuOpen)} style={styles.avatarCircle}>DR</button>
                        {isMenuOpen && (
                            <div style={styles.dropdownMenu}>
                                ...
                                <hr style={styles.menuDivider} />
                                <button onClick={() => { setIsMenuOpen(false); localStorage.removeItem('auth_token'); setAuth(false); }} style={{ ...styles.menuItem, ...styles.logoutItem }}>🚪 Log Out</button>
                            </div>
                        )}
                    </div>
                </div>
...
const styles = {
    appContainer: { backgroundColor: C.paper, minHeight: "100vh", color: C.ink, fontFamily: BODY },
    header: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 32px", background: C.card, borderBottom: `1px solid ${C.line}`, position: "sticky", top: 0, zIndex: 100 },
    logoSection: { display: "flex", alignItems: "center", gap: 10 },
    logoText: { fontFamily: DISPLAY, fontSize: "1.4rem", fontWeight: 700, margin: 0, color: C.ink },
    navActions: { display: "flex", alignItems: "center", gap: 12 },
    toggleBtn: { fontFamily: BODY, fontSize: 13, fontWeight: 600, padding: "8px 16px", borderRadius: 20, border: "1px solid", cursor: "pointer", transition: "all 0.15s ease" },
    profileContainer: { position: "relative" },
    avatarCircle: { width: "38px", height: "38px", borderRadius: "50%", backgroundColor: C.moss, color: "#ffffff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "600", fontSize: "0.9rem", border: "none", cursor: "pointer", boxShadow: "0 2px 6px rgba(27,49,28,0.15)" },
    dropdownMenu: { position: "absolute", top: "48px", right: "0", backgroundColor: "#ffffff", borderRadius: "12px", boxShadow: "0 8px 24px rgba(27, 49, 28, 0.12)", border: `1px solid ${C.line}`, width: "220px", padding: "8px 0", zIndex: 1000 },
    menuHeader: { padding: "12px 16px" },
    menuName: { margin: "0", fontWeight: "600", fontSize: "0.95rem", color: C.ink },
    menuEmail: { margin: "2px 0 0 0", fontSize: "0.8rem", color: C.subtext },
    menuDivider: { margin: "6px 0", border: "none", borderTop: `1px solid ${C.paper}` },
    menuItem: { display: "block", width: "100%", padding: "10px 16px", textAlign: "left", background: "none", border: "none", fontSize: "0.88rem", color: C.charcoal, cursor: "pointer", transition: "background-color 0.15s ease" },
    logoutItem: { color: C.danger, fontWeight: "600" },
    mainContent: { padding: "32px", maxWidth: "1300px", margin: "0 auto" },
    dashboardGrid: { display: "grid", gridTemplateColumns: "380px 1fr", gap: "24px", alignItems: "start" }
};
